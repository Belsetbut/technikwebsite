  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <div
          key={`empty-${i}`}
          className="h-24 border border-gray-200 bg-gray-50"></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">{day}</div>
          {dayEvents.map((event) => {
            const eventTime = new Date(event.startTime);
            const timeStr = eventTime.toLocaleTimeString("de-DE", {
              hour: "2-digit",
              minute: "2-digit",
            });
            return (
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="p-2 rounded-full hover:bg-blue-400/60 text-gray-700">
              →
            </button>
          </div>
        </div>
